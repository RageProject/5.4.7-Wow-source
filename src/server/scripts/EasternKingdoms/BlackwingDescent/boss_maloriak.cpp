            ...
            void FilterTargets(std::list<WorldObject*>& targets)
            {
                ...
                if (InstanceScript* pInstance = GetCaster()->GetInstanceScript())
                {
                    ...
                    if (!targets.empty() && count > 0)
                        TrinityCore::Containers::RandomResizeList(targets, count > 3? 3: count);
                    else
                        targets.clear();
                }
            }
        ...
        class spell_maloriak_release_all_minions_SpellScript : public SpellScript
        {
            ...
            { 
                ...
                if (InstanceScript* pInstance = GetCaster()->GetInstanceScript())
                {
                    ...
                    uint32 count = pInstance->GetData(DATA_MALORIAK_ABERRATIONS);
                    if (count > 0)
                        TrinityCore::Containers::RandomResizeList(targets, count);
                    else
                        targets.clear();
    ...
    public:
        ...
        class spell_maloriak_remedy_AuraScript : public AuraScript
        {
            ...
            void HandleTick(constAuraEffectPtr aurEff)
            {
                ...
                //if (baseAmount > 0)
                //    const_cast<AuraEffectPtr>(aurEff)->SetAmount(baseAmount * aurEff->GetTickNumber());
            }
