        ...
        class spell_saronite_vapors_AuraScript : public AuraScript
        {
            ...
            void HandleUpdatePeriodic(constAuraEffectPtr /*aurEff*/)
            {
                if (Unit* caster = ObjectAccessor::FindUnit(_caster))
                {
                    ...
                    TrinityCore::AnyPlayerInObjectRangeCheck u_check(caster, 150.0f, true);
                    TrinityCore::PlayerListSearcher<TrinityCore::AnyPlayerInObjectRangeCheck> searcher(caster, players, u_check);
                    caster->VisitNearbyObject(30.0f, searcher);
                    players.sort(TrinityCore::ObjectDistanceOrderPred(caster));
