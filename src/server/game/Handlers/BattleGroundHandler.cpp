...
void WorldSession::HandleBattleFieldPortOpcode(WorldPacket& recvData)
{
    ...
    {
        sLog->OutTrinityCore("HandleBattleFieldPortOpcode queueSlot %u", queueSlot);
        return;
    }
    ...
    {
        sLog->OutTrinityCore("HandleBattleFieldPortOpcode bgQueueTypeId %u", bgQueueTypeId);
        return;
    }
...
void WorldSession::HandleLeaveBattlefieldOpcode(WorldPacket& recvData)
{
    ...
    else if (_player->InBattlegroundQueue())
    {
        ...
        if (bgQueueTypeId >= MAX_BATTLEGROUND_QUEUE_TYPES)
        {
            sLog->OutTrinityCore("HandleLeaveBattlefieldOpcode: bgQueueTypeId %u", bgQueueTypeId);
            return;
        }
...
void WorldSession::HandleBattlemasterJoinArena(WorldPacket& recvData)
{
    ...
    for (GroupReference* itr = grp->GetFirstMember(); itr != NULL; itr = itr->next())
    {
        ...
        if (!ginfo)
        {
            sLog->OutTrinityCore("NULL ginfo !!!!");
            return;
        }
