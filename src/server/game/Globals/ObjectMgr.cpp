...
void ObjectMgr::AddCreatureToGrid(uint32 guid, CreatureData const* data)
{
    ...
    for (uint32 i = 0; mask != 0; i++, mask >>= 1)
    {
        if (mask & 1)
        {
            CellCoord cellCoord = TrinityCore::ComputeCellCoord(data->posX, data->posY);
    ...
    do
    {
        ...
        switch (tmp.command)
        {
            ...
            case SCRIPT_COMMAND_TELEPORT_TO:
            {
                ...
                if (!TrinityCore::IsValidMapCoord(tmp.TeleportTo.DestX, tmp.TeleportTo.DestY, tmp.TeleportTo.DestZ, tmp.TeleportTo.Orientation))
                {
            ...
            {
                if (!TrinityCore::IsValidMapCoord(tmp.TempSummonCreature.PosX, tmp.TempSummonCreature.PosY, tmp.TempSummonCreature.PosZ, tmp.TempSummonCreature.Orientation))
                {
...
void ObjectMgr::LoadPointsOfInterest()
{
    ...
    do
    {
        ...
        if (!TrinityCore::IsValidMapCoord(POI.x, POI.y))
        {
