...
uint16 ArchaeologyMgr::GetResearchSiteID()
{
    ...
    for (std::map<uint32, ResearchZoneEntry>::const_iterator itr = zoneMap.begin(); itr != zoneMap.end(); ++itr)
    {
        ...
        {
            if (TrinityCore::IsPointInZone(ResearchPOIPoint(_player->GetPositionX(), _player->GetPositionY()), itr->second.coords))
                return (*itr).first;
        }
    }
    return 0;
...
void ArchaeologyMgr::GenerateResearchSiteInMap(uint32 mapId, uint32 map)
{
    ...
    _researchSites[map].insert(TrinityCore::Containers::SelectRandomContainerElement(tempSites));
    _archaeologyChanged = true;
...
void ArchaeologyMgr::GenerateResearchSites()
{
    ...
    for (uint8 i = 0; i < 4; ++i)
        TrinityCore::Containers::RandomResizeSet(_researchSites[i], RESEARCH_SITES_PER_MAP);
...
bool ArchaeologyMgr::SolveResearchProject(uint32 projectId)
{
    ...
    _researchProjects.insert(TrinityCore::Containers::SelectRandomContainerElement(tempProjects));
    _archaeologyChanged = true;
