...
inline CellArea Cell::CalculateCellArea(float x, float y, float radius)
{
    if (radius <= 0.0f)
    {
        CellCoord center = TrinityCore::ComputeCellCoord(x, y).normalize();
        return CellArea(center, center);
    }

    CellCoord centerX = TrinityCore::ComputeCellCoord(x - radius, y - radius).normalize();
    CellCoord centerY = TrinityCore::ComputeCellCoord(x + radius, y + radius).normalize();
