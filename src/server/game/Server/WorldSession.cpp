...
#define MAX_PROCESSED_PACKETS_IN_SAME_WORLDSESSION_UPDATE 250
    ...
    if (sessionDiff > 70)
    {
        ...
        if (itr != pktHandle.end())
        {
            ...
            {
                sLog->OutTrinityCore("Account [%u] has been kicked for flood of CMSG_ADD_FRIEND (count : %u)", GetAccountId(), (*itr).second.nbPkt);
                KickPlayer();
                return false;
            }
        }

        sLog->OutTrinityCore("Session of account [%u] take more than 50 ms to execute (%u ms)", GetAccountId(), sessionDiff);
        for (auto itr : pktHandle)
            sLog->OutTrinityCore("-----> %u %s (%u ms)", itr.second.nbPkt, GetOpcodeNameForLogging((Opcodes)itr.first, WOW_CLIENT).c_str(), itr.second.totalTime);
    }
