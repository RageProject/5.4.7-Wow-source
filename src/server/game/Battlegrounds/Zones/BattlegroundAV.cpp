...
{
    return TrinityCore::Honor::hk_honor_at_level(m_MaxLevel, kills);
}
...
void BattlegroundAV::FillInitialWorldStates(WorldPacket& data)
{
    ...
    for (uint8 i = BG_AV_NODES_FIRSTAID_STATION; i <= BG_AV_NODES_FROSTWOLF_HUT; i++)
    {
        ...
        {//j=1=assaulted j=3=controled
            ...
            Player::AppendWorldState(data, uint32(BG_AV_NodeWorldStates[i][GetWorldStateType(j, ALLIANCE)]), uint32((m_Nodes[i].Owner == ALLIANCE && stateok) ? 1 : 0));
            Player::AppendWorldState(data, uint32(BG_AV_NodeWorldStates[i][GetWorldStateType(j, HORDE)]), uint32((m_Nodes[i].Owner == HORDE && stateok) ? 1 : 0));
        }
    }
    ...
    if (m_Nodes[BG_AV_NODES_SNOWFALL_GRAVE].Owner == AV_NEUTRAL_TEAM) //cause neutral teams aren't handled generic
        Player::AppendWorldState(data, uint32(AV_SNOWFALL_N), uint32(1));
    Player::AppendWorldState(data, uint32(AV_Alliance_Score), uint32(m_Team_Scores[0]));
    Player::AppendWorldState(data, uint32(AV_Horde_Score), uint32(m_Team_Scores[1]));
    if (GetStatus() == STATUS_IN_PROGRESS)
    { //only if game started the teamscores are displayed
        Player::AppendWorldState(data, uint32(AV_SHOW_A_SCORE), uint32(1));
        Player::AppendWorldState(data, uint32(AV_SHOW_H_SCORE), uint32(1));
    }
    else
    {
        Player::AppendWorldState(data, uint32(AV_SHOW_A_SCORE), uint32(0));
        Player::AppendWorldState(data, uint32(AV_SHOW_H_SCORE), uint32(0));
    }
